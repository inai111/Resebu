            ...
            @if (!empty($reseps))
                @foreach ($reseps as $resep)
                    <div class="row">
                        ...
                        <div class="col-auto">
                            <a href="{{ empty($resep->video) ? '/read/' . $resep->id : '/watch/' . $resep->id }}"
                                class="nav-link p-0 text-dark">
                                <h5>{{ $resep->nama }} <span
                                        class="text-primary">{{ $resep->kategori->kategori }}</span></h5>
                            </a>
                            <ol class="breadcrumb mb-4">
                                <li class="breadcrumb-item active">
                                    @if ($resep->komunitas)
                                        <a
                                            href="/komunitas/{{ $resep->komunitas->id }}">{{ ucwords($resep->komunitas->nama_komunitas) }}</a>
                                    @else
                                    by Chef {{ $resep->user->nama }}
                                    @endif 
                                </li>
